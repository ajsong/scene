...
class Encipher
{
	...
	/**
	 * Exclude the path not to encode.
	 */
    public $exclude_path = array();
	...
	/**
	 * echo the encode file path info.
	 */
	public $echo = false;

    public function __construct($source_file = '', $encoded_file = '', $comments = array(), $exclude_path = array()){
	    ...
	    !empty($exclude_path) && $this->exclude_path = (array)$exclude_path;
    ...
    public function encode($echo=false){
    	$this->echo = $echo;
    ...
    private function _encryptFiles($files){
        foreach($files as $file){
        	...
        	$is_encode = true;
        	if (count($this->exclude_path)) {
        		foreach ($this->exclude_path as $path) {
	        ...
	        if (!$is_encode || !strlen($type) || $type != 'php') {
	        	if (!file_exists($this->encoded_file . '/' . $file)) {
			        copy($this->source_file . '/' . $file, $this->encoded_file . '/' . $file);
		        }
	        	continue;
	        }
    ...
    private function _setHumanUnreadable($file){
        ...
        $headers = count($this->comments) ? array_map('trim', array_merge(array('<?php', '/*'), $this->comments, array('*/'))) : array('<?php');
