...
Page({
	...
	onLoad: function (options) {
        ...
        let _this = this;
        $.overload();
		$.get('/api/v2/buddha/video?special=1&id=' + options.id, function(json) {
			let video = json.data.video;
			let btn = json.data.btn;
			let addMy = json.data.addmy;
			let wxVideoAd = json.data.wxvideoad;
			//$.log($.screen().height)
			let shareHeight = 42;
    ...
    getData: function(isReachBottom, callback) {
        ...
        $.get('/api/v2/buddha/video?special=1&id=' + this.data.options.id, {
            offset: offset
        }, function(json) {
