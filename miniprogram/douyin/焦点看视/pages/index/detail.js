...
Page({
  ...
  onLoad: function (options) {
    ...
    $.get('/api/v2/article/detail?id=' + options.id, function (json) {
      ...
      if (Number(json.data.article.wxparse) == 1) {
        ...
        let nodes = $.nodes(json.data.article.content);
        json.data.article.nodes = nodes;
      }
      ...
      if (Number(json.data.newyear.enable) == 1) {
        ...
        if (json.data.newyear.music.length) {
          let audio = tt.createInnerAudioContext();
    ...
    tt.requestSubscribeMessage({
      ...
      fail: function (res) {
        switch (res.errCode) {
          ...
          case 10002:
            $.overloadError('网络问题，请求消息列表失败');
            break;

          case 10003:
            $.overloadError('网络问题，订阅请求发送失败');
            break;

          case 10004:
            $.overloadError('参数类型错误');
            break;

          case 10005:
            $.overloadError('无法展示 UI，一般是小程序这个时候退后台了导致的');
            break;

          case 20001:
            $.overloadError('没有模板数据，一般是模板 ID 不存在 或者和模板类型不对应 导致的');
            break;

          case 20002:
            $.overloadError('模板消息类型 既有一次性的又有永久的');
            break;

          case 20003:
            $.overloadError('模板消息数量超过上限');
            break;

          case 20004:
            $.overloadError('用户关闭了主开关，无法进行订阅');
            break;

          case 20005:
            $.overloadError('小程序被禁封');
            break;
        }
      }
    });
  ...
  handleFeedbackSubmit: function () {
    ...
    $.post('/api/v2/other/feedback', {
      content: this.data.feedbackContent,
      parent_id: this.data.data.id
    }, function () {
  ...
  handleCommentSend: function () {
    ...
    $.post('/api/v2/article/comment', {
      article_id: this.data.data.id,
