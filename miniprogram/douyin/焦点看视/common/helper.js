...
module.exports = {
  ...
  envVersion: tt.env ? tt.env.VERSION : '',
  //develop开发版，trial体验版，release正式版
  //wxSDKVersion: tt.getSystemInfoSync().SDKVersion,
  log: console.log,
  WxParse: WxParse,
  //使用 rich-text 代替性能会更好
  md5: function (str) {
  ...
  get: function (url, data, callback) {
    ...
    if (tt.getAccountInfoSync) header['Appid'] = tt.getAccountInfoSync().miniProgram.appId;
  ...
  upload: function (url, options, callback, progress) {
    ...
    if (this.isFunction(progress)) {
      task.onProgressUpdate(function (res) {
        progress(res); //res.progress//进度百分比
  ...
  checkLogin: function (url) {
    let person = this.storage('person'); //console.log(person)

    if (person) {
      if (typeof url != 'undefined' && url.length) tt.navigateTo({
        url: url
      });
      return true;
    } else {
  ...
  el: function (expr, callback) {
    let $ = this;
    tt.createSelectorQuery().select(expr).boundingClientRect(function (res) {
  ...
  find: function (expr, callback) {
    let $ = this;
    tt.createSelectorQuery().selectAll(expr).boundingClientRect(function (res) {
      if ($.isFunction(callback)) callback(res);
  ...
  relaunchView: function (url) {
    if (typeof url != 'undefined' && url.length) tt.reLaunch({
      url: url
    });
  },
  //关闭当前页面,跳转到应用内的某个页面
  redirectView: function (url) {
    if (typeof url != 'undefined' && url.length) tt.redirectTo({
      url: url
    });
  },
  //跳转页面
  ...
  getLocation: function (callback, fail, type) {
    ...
    tt.getLocation({
      type: type,
      //wgs84返回gps坐标，gcj02返回可用于wx.openLocation的坐标
      success: function (res) {
        $.baiduAddress({
  ...
  createMap: function (context) {
    return tt.createMapContext(context);
  },
  ...
  switchView: function (options) {
    ...
    switchView = $.extend(switchView, {
      list: [],
      //列表,格式:[{name:'选项名',value:'选项值',cls:'附加样式'}]
      selected: '',
      //默认选中项,值为选项值
      click: null,
      //点击后执行,三个参数:选项值、选项名、选项索引
      switchViewWidth: 0
    }, options);
    app.setData({
  ...
  loginAuthApi: function (userInfo, callback) {
    ...
    $.getOpenid(function (openid) {
      ...
      $.post('/api/?app=core&act=weixin_auth' + (reseller ? '&reseller=' + reseller : ''), {
  ...
  dialogView: function (options) {
    ...
    options = this.extend({
      cls: 'wx',
      //附加样式
      showIn: 'dialog-scale-in',
      //动画样式
      showOut: 'dialog-scale-out',
      //关闭动画样式
      title: '',
      //标题,为空即不显示
      content: '',
