...
function extend() {
  for (var i = arguments.length; i > 0; i--) {
    for (var k in arguments[i]) {
      arguments[i - 1][k] = clone(arguments[i][k]);
    }
  }
...
function html2json(html, bindName) {
  ...
  HTMLParser(html, {
    start: function (tag, attrs, unary) {
      ...
      if (attrs.length !== 0) {
        node.attr = attrs.reduce(function (pre, attr) {
          ...
          if (name == 'style') {
            // console.dir(value);
            //  value = value.join("")
            value = value.replace(/(-?[\d\.]+)(px|rem)\b/ig, function ($, $1, $2) {
              var number = Number($1);
      ...
      if (node.tag === 'input') {
        var type = 'text';
        if (node.attr.type) type = node.attr.type;
        if (type == 'tel') type = 'digit';else if (type == 'number') type = 'number';else type = 'text';
        node.attr.type = type;
      } //处理textarea
      ...
      if (node.tag === 'textarea') {
        node.from = bindName;
      } //处理select,需要增加text自定义属性存放默认显示文本


      if (node.tag === 'select') {
        node.from = bindName;
      } //处理select的option,需要option增加text自定义属性存放显示文本


      if (node.tag === 'option') {
        node.text = 'undefined';

        if (node.attr && node.attr.text) {
          node.text = node.attr.text;
        }
      } //对img添加额外数据
      ...
      if (node.tag === 'img') {
        ...
        node.attr.notap = node.attr['data-notap'] ? true : false;
        node.from = bindName;
      ...
      if (node.tag === 'font') {
        ...
        if (node.attr) for (var key in styleAttrs) {
          if (node.attr[key]) {
