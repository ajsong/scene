/*
Developed by @mario v3.0.20210103
*/
let config = require('config.js'),
	WxParse = require('libs/wxParse/wxParse.js'),
	HtmlToJson = require('libs/wxParse/html2json.js'),
	MD5 = require('libs/md5.js');
module.exports = {
	//配置参数
	log: console.log,
	config: config,
	extConfig: typeof (wx.getExtConfigSync) !== 'undefined' ? wx.getExtConfigSync() : {},
	wxConfig: typeof (__wxConfig) !== 'undefined' ? __wxConfig : {},
	envVersion: typeof (__wxConfig.envVersion) !== 'undefined' ? __wxConfig.envVersion : '', //develop开发版, trial体验版, release正式版
	wxSDKVersion: wx.getSystemInfoSync().SDKVersion,
	WxParse: WxParse, //使用 rich-text 性能会更好
	md5: function(str) {return MD5.hexMD5(String(str))},
	//网络请求回调(成功)
	...
	//DEBUG收集
	debugApi: function(data, callback) {
		let $ = this, api = $.storage('debug_api');
		if (api === null) return this;
		if (!config.apiUrl.length) {
			setTimeout(function(){
				$.debugApi(data, callback);
			}, 300);
			return this;
		}
	...
	get: function(url, data, callback) {
		...
		if ($.isPlainObject(param) && !$.isEmptyObject(param)) {
			for (let key in param) {
				url += (url.indexOf('?') > -1 ? '&' : '?') + key + '=' + $.urlencode(param[key]);
			}
		}
		if (_app) {
			let headerParam = _app.headerParam;
			if (headerParam) {
				if ($.isFunction(headerParam)) {
					headerParam = headerParam();
				}
				if ($.isPlainObject(headerParam) && !$.isEmptyObject(headerParam)) {
					for (let key in headerParam) {
						header[key] = headerParam[key];
					}
				}
			}
		}
		...
		if (typeof (wx.getAccountInfoSync) !== 'undefined') header['Appid'] = wx.getAccountInfoSync().miniProgram.appId;
	...
	setTabBarBadge: function(json, tabIndexs) {
		if (typeof (tabIndexs) === 'undefined') return this;
		if (json.red_dot) {
	...
	isX: function() {
		try {
			...
			if (res.safeArea.top > 20 || res.model.toLowerCase().indexOf('iphone x') > -1) {
				return true;
			} else {
				return false;
			}
	...
	isPlainObject: function(obj) {
		if (!obj) return false;
		if (obj && typeof (obj) === 'object' && Object.prototype.toString.call(obj).toLowerCase() === '[object object]') return true;
		return false;
	},
	//是否空对象
	...
	switchView: function(options) {
		...
		if ($.isFunction(switchView.complete)) {
			setTimeout(function(){
				switchView.complete();
			}, 150);
		}
	...
	loginAuthApi: function(userInfo, callback) {
		...
		return $.getOpenid(function(openid) {
			$.overload();
			userInfo.openid = openid;
			$.post('/api/core/weixin_auth' + (reseller ? '?reseller=' + reseller : ''), {
	...
	overload: function(text, icon, delay) {
		...
		if (typeof (text) === 'boolean' && !text) {
			if (overload.delay > 0) return this;
	...
	overloadSuccess: function(text) {
		...
		return this.overload(text, 'load-success', delay);
	},
	overloadError: function(text) {
		...
		return this.overload(text, 'load-error', delay);
	},
	overloadWarning: function(text) {
		...
		return this.overload(text, 'load-warning', delay);
	},
	//自定义内容弹框
	dialogView: function(options) {
		...
		if (options.cls === 'wxauth' || options.cls === 'image-wxauth') {
			console.loginAuth(options.cls);
			return this;
		}
		if (options.cls === 'dialog-wxauth' && $.storage('WxauthCancel')) return this;
		let app = apps[apps.length - 1],
		...
		app.dialogViewHandler = function(e) {
			...
			if ($.isFunction(options.callback)) {
				...
				if (typeof (result) === 'undefined' || (typeof (result) === 'boolean' && result)) app.dialogViewCancel();
	...
	toast: function (options, cls) {
		...
		if (typeof (cls) === 'string') opt = $.extend(opt, {cls: cls});
		let showInCls = opt.showIn;
		app.setData({
			toast: $.extend(opt, {
				show: true,
				showIn: '',
		...
		setTimeout(function () {
			...
			}, opt.text.length * (opt.text.length <= 5 ? 1000 : 300));
		}, 50);
		return this;
	},
	...
	comboCity: function(province, city, district, address, apart) {
		...
		let str = province;
		if (typeof (city) !== 'undefined' && province !== city) str += apart + city;
		if (typeof (district) !== 'undefined') str += apart + district;
		if (typeof (address) !== 'undefined') str += apart + address;
		return str;
	},
	//加上天数得到日期
	...
	formatDate: function(date, formatStr, callback) {
		...
		let $ = this,
			format = typeof (formatStr) !== 'undefined' ? formatStr : 'yyyy-mm-dd hh:nn:ss',
	...
	//html转rich-text用的nodes格式
	nodes: function(data) {
		let $ = this, d = HtmlToJson.html2json(data), recursion = function(nodes){
			let items = [];
			try { //forEach跳出循环必须使用try
				nodes.forEach(function(g){
					let item = {};
					if (typeof (g.text) !== 'undefined') {
						if (!$.trim(g.text).length) return; //continue
						item.type = 'text';
						item.text = g.text;
					} else {
						if (!$.trim(g.tag).length) throw new Error('Break'); //break
						item.name = g.tag;
						if (typeof (g.attr) !== 'undefined') {
							for (let key in g.attr) {
								if (key === 'notap') delete g.attr[key];
								if (key.indexOf('-') > -1) delete g.attr[key];
							}
							if (typeof (g.styleStr) !== 'undefined') {
								g.attr.style = String(g.styleStr);
							}
							item.attrs = g.attr;
						} else if (typeof (g.styleStr) !== 'undefined') {
							let attr = {style: String(g.styleStr)};
							item.attrs = attr;
						}
						if (typeof (g.nodes) !== 'undefined' && $.isArray(g.nodes) && g.nodes.length) {
							item.children = recursion(g.nodes);
						}
					}
					items.push(item);
				});
