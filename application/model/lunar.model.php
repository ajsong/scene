...
class lunar_model
{
	...
	*/
	function getFestival($today, $nl_info = false,$config = 1)
	{
		...
		if(isset($arr_lunar[$md_lunar]))array_push($festivals, $arr_lunar[$md_lunar]);
		if(isset($arr_solar[$md_solar]))array_push($festivals, $arr_solar[$md_solar]);
	...
	* 获取当前时间属于哪个时辰
	@param int $time  时间戳
	*/
	function getTheHour($time){
		$d=intval(date('H', $time));
		if($d==23 || $d==0){
			return '子时';
		}else if($d==1 || $d==2){
			return '丑时';
...
/*$lunar=new Lunar();//
$month=$lunar->convertSolarToLunar(2017,05,13);//将阳历转换为阴历
echo '<pre>';
print_r($month);*/