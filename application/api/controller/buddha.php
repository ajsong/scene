...
class buddha extends core {
	...
	public function __construct() {
		...
		if ($this->act != 'pickBuddha') {
			...
			if (!SQL::share('miniprogram_config')->where("miniprogram_id='{$this->miniprogram->id}'")->exist()) {
				$rs = SQL::share('config')->where("name LIKE 'G_BUDDHA/_%' ESCAPE '/'")->find('id, content');
				if ($rs) {
					$data = array();
					foreach ($rs as $row) {
			...
			$rs = SQL::share('config c')->left('miniprogram_config mc', 'mc.config_id=c.id')->where("c.name LIKE 'G_BUDDHA/_%' ESCAPE '/' AND mc.miniprogram_id='{$this->miniprogram->id}'")->find('c.name, mc.content');
			if ($rs) {
				$configs = array();
				foreach ($rs as $row) {
			...
			$this->indexListAdEnable = $this->request->act('G_BUDDHA_INDEX_LIST_AD_ENABLE', 0, '', $configs);
			$this->indexListAdImage = $this->request->act('G_BUDDHA_INDEX_LIST_AD_IMAGE', '', '', $configs);
			$this->indexListAdUrl = $this->request->act('G_BUDDHA_INDEX_LIST_AD_URL', '', '', $configs);
			$this->indexAddMyEnable = $this->request->act('G_BUDDHA_INDEX_ADDMY_ENABLE', 0, '', $configs);
			$this->indexAddMyBgColor = $this->request->act('G_BUDDHA_INDEX_ADDMY_BGCOLOR', '', '', $configs);
			$this->detailTopAdEnable = $this->request->act('G_BUDDHA_DETAIL_TOP_AD_ENABLE', 0, '', $configs);
			$this->detailTopAdType = $this->request->act('G_BUDDHA_DETAIL_TOP_AD_TYPE', 0, '', $configs);
			$this->detailTopAdImage = $this->request->act('G_BUDDHA_DETAIL_TOP_AD_IMAGE', '', '', $configs);
			$this->detailTopAdUrl = $this->request->act('G_BUDDHA_DETAIL_TOP_AD_URL', '', '', $configs);
			$this->detailTopAdUnitId = $this->request->act('G_BUDDHA_DETAIL_TOP_AD_UNITID', '', '', $configs);
			$this->detailBottomAdEnable = $this->request->act('G_BUDDHA_DETAIL_BOTTOM_AD_ENABLE', 0, '', $configs);
			$this->detailBottomAdType = $this->request->act('G_BUDDHA_DETAIL_BOTTOM_AD_TYPE', 0, '', $configs);
			$this->detailBottomAdImage = $this->request->act('G_BUDDHA_DETAIL_BOTTOM_AD_IMAGE', '', '', $configs);
			$this->detailBottomAdUrl = $this->request->act('G_BUDDHA_DETAIL_BOTTOM_AD_URL', '', '', $configs);
			$this->detailBottomAdUnitId = $this->request->act('G_BUDDHA_DETAIL_BOTTOM_AD_UNITID', '', '', $configs);
			$this->detailBtnEnable = $this->request->act('G_BUDDHA_DETAIL_BTN_ENABLE', 0, '', $configs);
			$this->detailBtnText = $this->request->act('G_BUDDHA_DETAIL_BTN_TEXT', '', '', $configs);
			$this->detailBtnUrl = $this->request->act('G_BUDDHA_DETAIL_BTN_URL', '', '', $configs);
			$this->detailListAdEnable = $this->request->act('G_BUDDHA_DETAIL_LIST_AD_ENABLE', 0, '', $configs);
			$this->detailListAdImage = $this->request->act('G_BUDDHA_DETAIL_LIST_AD_IMAGE', '', '', $configs);
			$this->detailListAdUrl = $this->request->act('G_BUDDHA_DETAIL_LIST_AD_URL', '', '', $configs);
			$this->returnEnable = $this->request->act('G_BUDDHA_RETURN_ENABLE', 0, '', $configs);
			$this->returnEverybody = $this->request->act('G_BUDDHA_RETURN_EVERYBODY', 0, '', $configs);
			$this->returnUrls = $this->request->act('G_BUDDHA_RETURN_URLS', '', '', $configs);
			$this->detailTipsEnable = $this->request->act('G_BUDDHA_DETAIL_TIPS_ENABLE', 0, '', $configs);
			$this->detailTipsText = $this->request->act('G_BUDDHA_DETAIL_TIPS_TEXT', '', '', $configs);
			$this->detailTipsUrl = $this->request->act('G_BUDDHA_DETAIL_TIPS_URL', '', '', $configs);
			$this->detailTipsPosition = $this->request->act('G_BUDDHA_DETAIL_TIPS_POSITION_ENABLE', 0, '', $configs);
			$this->detailTipsRedirect = $this->request->act('G_BUDDHA_DETAIL_TIPS_REDIRECT', '', '', $configs);
			$this->detailNewYearEnable = $this->request->act('G_BUDDHA_DETAIL_NEW_YEAR_ENABLE', 0, '', $configs);
			$this->detailNewYearMusic = $this->request->act('G_BUDDHA_DETAIL_NEW_YEAR_MUSIC', '', '', $configs);
			$this->detailNewYearMusic2 = $this->request->act('G_BUDDHA_DETAIL_NEW_YEAR_MUSIC2', '', '', $configs);
			$this->detailPositionEnable = $this->request->act('G_BUDDHA_DETAIL_POSITION_ENABLE', 0, '', $configs);
			$this->detailPositionType = $this->request->act('G_BUDDHA_DETAIL_POSITION_TYPE', 0, '', $configs);
			$this->detailPositionText = $this->request->act('G_BUDDHA_DETAIL_POSITION_TEXT', '', '', $configs);
			$this->detailPositionUrl = $this->request->act('G_BUDDHA_DETAIL_POSITION_URL', '', '', $configs);
			$this->detailPosAdUrl = $this->request->act('G_BUDDHA_DETAIL_POSAD_URL', '', '', $configs);
			$this->detailPosAdImage = $this->request->act('G_BUDDHA_DETAIL_POSAD_IMAGE', '', '', $configs);
			$this->detailAddMyEnable = $this->request->act('G_BUDDHA_DETAIL_ADDMY_ENABLE', 0, '', $configs);
			$this->detailFeedbackEnable = $this->request->act('G_BUDDHA_DETAIL_FEEDBACK', 0, '', $configs);
			$this->wxAdEnable = $this->request->act('G_BUDDHA_WX_AD_ENABLE', 0, '', $configs);
			$this->indexListWxAdEnable = $this->request->act('G_BUDDHA_INDEX_LIST_WX_AD_ENABLE', 0, '', $configs);
			$this->indexListWxAdUnitId = $this->request->act('G_BUDDHA_INDEX_LIST_WX_AD_UNITID', '', '', $configs);
			$this->indexListWxVideoAdUnitId = $this->request->act('G_BUDDHA_INDEX_LIST_WX_VIDEO_AD_UNITID', '', '', $configs);
			$this->detailListWxAdEnable = $this->request->act('G_BUDDHA_DETAIL_LIST_WX_AD_ENABLE', 0, '', $configs);
			$this->detailListWxAdUnitId = $this->request->act('G_BUDDHA_DETAIL_LIST_WX_AD_UNITID', '', '', $configs);
			$this->detailListWxVideoAdUnitId = $this->request->act('G_BUDDHA_DETAIL_LIST_WX_VIDEO_AD_UNITID', '', '', $configs);
			$this->detailPositionWxAdEnable = $this->request->act('G_BUDDHA_DETAIL_POSITION_WX_AD_ENABLE', 0, '', $configs);
			$this->detailPositionWxAdUnitId = $this->request->act('G_BUDDHA_DETAIL_POSITION_WX_AD_UNITID', '', '', $configs);
			$this->indexPositionWxAdEnable = $this->request->act('G_BUDDHA_INDEX_POSITION_WX_AD_ENABLE', 0, '', $configs);
			$this->indexPositionWxAdUnitId = $this->request->act('G_BUDDHA_INDEX_POSITION_WX_AD_UNITID', '', '', $configs);
			$this->customMessageTitle = $this->request->act('G_BUDDHA_CUSTOM_MESSAGE_TITLE', '', '', $configs);
			$this->customMessagePath = $this->request->act('G_BUDDHA_CUSTOM_MESSAGE_PATH', '', '', $configs);
			$this->customMessageImg = $this->request->act('G_BUDDHA_CUSTOM_MESSAGE_IMG', '', '', $configs);
	...
	public function categories() {
		...
		$categories = SQL::share('buddha_category')->where("status=1")->sort('sort ASC')->cached(60*5)->find();
		...
		success(compact('categories', 'addmy', 'audio', 'bg', 'font'), '成功', 0, ['appId'=>$this->appId]);
	}
	
	//分类
	public function category() {
		$id = $this->request->get('id', 0);
		$category = SQL::share('buddha_category')->where($id)->cached(60*5)->row();
...
//		$cate3->name = '精选';
		...
		if ($this->miniprogram->review==0) {
			$cates = SQL::share('buddha_category')->where("status=1")->sort('sort ASC, id ASC')->find('id, name');
			foreach ($cates as $g) {
				$categories[] = $g;
			}
		}
...
//			$categories[] = $cate4;
		...
		$data = compact('res', 'addmy', 'addmybgcolor', 'flashes', 'categories', 'list', 'wxpositionad', 'bg');
		if ($this->ver > 2) {
	...
	public function buddhaList() {
		...
		$styleIndex = 0;
		$list = [];
		$rs = SQL::share('buddha a')->left('buddha_attr aa', 'a.id=aa.buddha_id')
			->where("status=1 {$where}")->sort('aa.clicks DESC, sort ASC, a.id DESC')->limit($offset, $pagesize)// AND more=1
	...
	public function detail() {
		...
		if ($this->ver > 2) {
			...
			$count = SQL::share('buddha a')->left('buddha_attr aa', 'a.id=aa.buddha_id')->where($w)->sort('a.id DESC')->count();
			if ($offset==0) {
		...
		if ($this->miniprogram->early==0) {
			$rs = SQL::share('buddha_comment')
				->where("miniprogram_id='{$this->miniprogram->id}' AND buddha_id='{$id}' AND status=1")->sort('id DESC')->limit($offset, $pagesize)->find("*, '' as member_name, '' as member_avatar");
			if ($rs) {
				foreach ($rs as $g) {
					$g->member_name = '网友';
		...
		if ($this->miniprogram->early==1) {
			if (strlen($this->detailTipsUrl)) {
				$ids = SQL::share('buddha a')->left('buddha_attr aa', 'a.id=aa.buddha_id')->where("a.status=1 AND a.id!='{$id}' AND a.type=0")->sort('aa.clicks DESC')->pagesize(10)->returnArray()->find('a.id');
				if (count($ids)) {
					shuffle($ids);
		...
		if ($this->ver > 2) {
			...
			$prev_page = SQL::share('buddha a')->left('buddha_attr aa', 'a.id=aa.buddha_id')->where($where)->sort('a.id ASC')->row('a.id, a.title, type, music');
			$where = "aa.miniprogram_id='{$this->miniprogram->id}' AND status=1 AND category_id='{$buddha->category_id}'";
			$where .= " AND a.id<'{$id}' AND a.type=0 AND a.music_enable=1";
			$next_page = SQL::share('buddha a')->left('buddha_attr aa', 'a.id=aa.buddha_id')->where($where)->sort('a.id DESC')->row('a.id, a.title, type, music');
		}
		$bg = '/uploads/pic/2020/05/29/20052916064558550.jpg';
		$bgsize = '320px 210px';
		success(compact('buddha', 'content_next', 'banner', 'footer', 'btn', 'tips', 'newyear', 'position', 'posad', 'trans', 'addmy', 'feedback', 'ad_fixed', 'ad_fixed_percent', 'subscribe_id', 'comment_hidden', 'wxpositionad', 'list', 'comment', 'prev_page', 'next_page', 'bg', 'bgsize'), '成功', 0, ['appId'=>$this->appId]);
	}
	...
	public function video() {
		...
		$video = SQL::share('buddha a')->left('buddha_attr aa', 'a.id=aa.buddha_id')->where("a.id='{$id}' AND aa.miniprogram_id='{$this->miniprogram->id}'")
			->row('a.*, aa.clicks, aa.likes');
		if (!$video) error('记录不存在');
		SQL::share('buddha_attr')->where("buddha_id='{$id}' AND miniprogram_id='{$this->miniprogram->id}'")->update(['clicks'=>['+1']]);
		SQL::share('buddha')->where($id)->update(['clicks'=>['+1'], 'today_clicks'=>['+1']]);
	...
	public function like() {
		...
		if ($id<=0) error('文章不存在');
		SQL::share('buddha')->where($id)->update(['likes'=>['+1']]);
		SQL::share('buddha_attr')->where("buddha_id='{$id}' AND miniprogram_id='{$this->miniprogram->id}'")->update(['likes'=>['+1']]);
		success('ok');
	}
	
	public function comment() {
		if (IS_POST) {
			$buddha_id = $this->request->post('buddha_id', 0);
			$content = $this->request->post('content');
			if ($buddha_id<=0 || !strlen($content)) error('缺少参数');
			$count = SQL::share('buddha_comment')->where("ip='{$this->ip}'")->comparetime('n', 'add_time', '=0')->count();
			if ($count>=2) error('您发表得太快了');
			$miniprogram_id = $this->miniprogram->id;
			$ip = $this->ip;
			$add_time = time();
			$row = SQL::share('buddha_comment')->returnObj()->insert(compact('miniprogram_id', 'buddha_id', 'content', 'ip', 'add_time'));
			$row->member_name = '网友';
			$row->member_avatar = '/images/avatar.png';
			$row->add_time = get_time_word($row->add_time);
			$row->type = 0;
			$row = add_domain_deep($row, ['member_avatar']);
			SQL::share('buddha')->where($buddha_id)->update(['comments'=>['+1']]);
			success($row);
		}
		success('ok');
	}
	...
	public function comment_like() {
		$id = $this->request->post('id', 0);
		if ($id<=0) error('评论不存在');
		SQL::share('buddha_comment')->where($id)->update(['likes'=>['+1']]);
		success('ok');
	}
	...
	public function pickBuddha() {
		...
		foreach ($categories as $cateid) {
			...
			if (is_array($json['data']['list'])) {
				...
				foreach ($list as $l) {
					if (SQL::share('buddha')->where("title='".$l['title']."'")->exist()) continue;
					$rs = SQL::share('buddha')->find('title');
					if ($rs) {
						$similar = false;
						foreach ($rs as $g) {
					...
					$buddha_id = SQL::share('buddha')->insert($r);
					$miniprogram_id = 0;
					SQL::share('buddha_attr')->insert(compact('miniprogram_id', 'buddha_id'));
					$count++;
				}
			}
		}
	...
	private function _getFile($url, $suffix='') {
		...
		curl_close($ch);
		$file = upload_obj_file($content, 'buddha');
