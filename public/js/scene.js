...
function setPic(obj, nonLoadImage){
	let viewer = $('.wap-detail > ul > li:last'), width = viewer.width(), url = obj.url, style = (typeof obj.style==='undefined' ? '' : obj.style), image = new Image();
	image.src = url;
	let s = function(){
...
function setText(obj){
	let viewer = $('.wap-detail > ul > li:last'), text = (typeof obj.text==='undefined' ? '' : obj.text), style = (typeof obj.style==='undefined' ? '' : obj.style);
...
function setVideo(obj){
	let viewer = $('.wap-detail > ul > li:last'), code = obj.code||'', style = (typeof obj.style==='undefined' ? '' : obj.style);
...
function setWeb(obj){
	let viewer = $('.wap-detail > ul > li:last'), url = obj.url||'', style = (typeof obj.style==='undefined' ? '' : obj.style);
...
function setMap(obj){
	let viewer = $('.wap-detail > ul > li:last'), style = (typeof obj.style==='undefined' ? '' : obj.style),
	...
	try{
		...
		if($.isArray(window.scenePages)){
			...
			if(window.scenePages.length>1){
				let wrap = $('.wap-detail'), domain = $('#oss_domain').length ? $('#oss_domain').val() : '';
