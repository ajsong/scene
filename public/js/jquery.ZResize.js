	...
	ZResize.prototype = {
		...
		init: function() {
			...
			if(self.options.guide) {
				let targetParent = $(self.options.target).parent();
		...
		skipOperate: function(e, org) {
			let self = this;
			if($.isFunction(self.options.skipOperate)) return self.options.skipOperate.call(org, e);
			return false;
		},
		/**
		...
		bindResizeEvent: function(el) {
			...
			//拖拽
			let drag = false;
			let rotate = false, preAngle = 0, preX = 0, preY = 0, degrees = 0, orgOffset = {left:0, top:0};
			el.on('mousedown', function(e) {
				...
				if(self.options.guide) self.guides = $.map(org.siblings().not(self.options.unusualGuide), self.guidesForElement);
				started = true;
				ox = e.pageX;
				oy = e.pageY;
				ow = el.outerWidth(false);
			...
			if(self.options.range) {
				$(self.options.range).on('mouseleave', function() {
					emove = false;
					smove = false;
					wmove = false;
					nmove = false;
